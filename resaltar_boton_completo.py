...
def main():
    ...
    with sync_playwright() as p:
        try:
            ...
            # Guardar evidencia de la página de login
            guardar_evidencia_html(page, "pagina_login", directorio_evidencias)
            
            # Paso 2: Navegar a la página de posición global
            logging.info("Navegando a la página de posición global")
            page.goto("https://particulares.bancosantander.es/oneweb/global-position", timeout=30000)
            page.wait_for_timeout(5000)
            
            # Guardar evidencia de la página de posición global
            guardar_evidencia_html(page, "pagina_global_position", directorio_evidencias)
            
            # Paso 3: Hacer clic en la primera cuenta (esto requeriría credenciales reales)
            # Como no tenemos credenciales, vamos directamente a la página de cuentas
            ...
            # Guardar evidencia de la página de cuentas antes de buscar el botón
            guardar_evidencia_html(page, "pagina_cuentas_antes", directorio_evidencias)
            
            # Paso 4: Buscar y resaltar el botón de descarga
            boton_resaltado = resaltar_boton_descargar(page)
            
            if boton_resaltado:
                # Guardar evidencia HTML después de resaltar el botón
                ruta_evidencia = guardar_evidencia_html(page, "pagina_cuentas_despues", directorio_evidencias)
                ...
                ruta_evidencia = guardar_evidencia_html(page, "pagina_cuentas_sin_boton", directorio_evidencias)
                if ruta_evidencia:
