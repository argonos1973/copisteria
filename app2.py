...
def obtener_factura_para_imprimir(factura_id):
    try:
        ...
        # Recalcular BASE/IVA/TOTAL con redondeo por línea (ROUND_HALF_UP, 2 decimales)
        # para evitar discrepancias de coma flotante (p.ej. 56,11 vs 56,12)
        ...
        for r in resultados_dict:
            qty = D(r.get('cantidad'))
            price = D(r.get('precio'))
            sub = qty * price
        ...
        # Calcular IVA sobre la base total (no por líneas)
        tax_rate = D('21')  # Asumimos 21% para todas las líneas
        ...
        factura = {
            ...
            # Usar los valores recalculados para asegurar consistencia de redondeo
            'base': float(base_sum),
            'iva': float(iva_sum),
        ...
        for resultado in resultados_dict:
            if resultado['concepto'] or resultado['descripcion']:  # Solo si hay concepto o descripción (evita líneas nulas)
                # Calcular subtotal SIN IVA para mostrar en las líneas
                ...
                subtotal_sin_iva = round(cantidad * precio, 2)
                ...
                linea = {
                    ...
                    'total': subtotal_sin_iva,  # Subtotal SIN IVA
