...
with sync_playwright() as p:
    ...
    try:
        ...
        if cuentas.count() > 0:
            ...
            if cuenta_btn.is_visible():
                ...
                cuenta_btn.click()
                # Esperar a que la página de movimientos termine de cargar
                page.wait_for_load_state("networkidle")
                page.wait_for_timeout(3000)
                ...
                if exportar_excel_btn.is_visible():
                    ...
                    try:
                        ...
                        if descargar_excel_link.is_visible():
                            ...
                            print(f"Error durante la descarga del Excel: {e}")
                    except Exception as e:
    ...
    # Intentar hacer click en el icono de descarga tras seleccionar la cuenta
    try:
        print("Buscando icono de descarga tras seleccionar la cuenta ...")
        # Esperar a que termine la carga de la página
        page.wait_for_load_state("networkidle")
        page.wait_for_timeout(2000)
        # Captura de pantalla para debug
        try:
            page.screenshot(path="/tmp/debug_download.png", full_page=True)
            print("Screenshot guardado en /tmp/debug_download.png")
        except Exception as e:
            print(f"Error al capturar screenshot: {e}")
        # Primer intento: esperar explícitamente div.download_icon y hacer clic directamente
        # Esperar un poco para que la UI termine de renderizar
        page.wait_for_timeout(2000)
        # Desplazarse al final y volver al principio para forzar la carga perezosa de iconos
        try:
        ...
        # Primer intento: esperar explícitamente div.download_icon y hacer clic directamente
        try:
            page.wait_for_selector("div.download_icon", state="visible", timeout=30000)
            dl_div = page.locator("div.download_icon").first
            dl_em = dl_div.locator("em").first
            dl_em.click(force=True)
            print("Click directo en download_icon realizado")
            page.wait_for_timeout(3000)
            download_icon_pre = None  # ya manejado
        except Exception as e:
            print(f"Primer intento download_icon falló: {e}")

        if download_icon_pre is None:
            icon_selectors = [
            "div.download_icon em.c-sanicon__download",
            "div.download_icon em[class*='download']",
            "div.buttons em[class*='download']",
            "em.c-sanicon__download",
            "em[class*='download']"
            ]
            download_icon_pre = None
            for sel in icon_selectors:
                try:
                    candidate = page.locator(sel).first
                    # Esperar a que exista en el DOM sin exigir visibilidad
                    if candidate.count() == 0:
                        page.wait_for_selector(sel, state="attached", timeout=30000)
                        candidate = page.locator(sel).first
                    # Aceptamos incluso si no es visible
                    download_icon_pre = candidate
                    break
                except Exception:
                    continue
        if download_icon_pre:
            try:
                download_icon_pre.scroll_into_view_if_needed()
            except Exception:
                pass
            print("Clickando icono de descarga (force=True) ...")
            try:
                download_icon_pre.hover(timeout=2000)
            except Exception:
                pass
            download_icon_pre.click(force=True)
            page.wait_for_timeout(3000)
        else:
            print("No se pudo localizar el icono de descarga tras todos los intentos")

    except Exception as e:
        print(f"No se pudo hacer clic en el icono de descarga: {e}")

    """ Se omiten filtros de fecha y botones de búsqueda; bloque comentado """
    """
    fecha_fin_selectores = []
    
    fecha_inicio_ok = False
    fecha_fin_ok = False
    
    # Deshabilitado bucle filtros fecha inicio
        try:
    ...
    # Deshabilitado bucle filtros fecha fin
        try:
    ...
    # No se aplican filtros de fecha
    ...
    buscar_selectores = []  # Sin botón buscar
    ...
    try:
        ...
        print("No se encontró el icono de descarga visible.")
                            ...
                            import pandas as pd
                            # import sqlite3  # Eliminado: sin operaciones de BD
                            import os
                            try:
                                ...
                                # conn = get_db_connection()  # Eliminado
                                # cursor = conn.cursor()  # Eliminado
                                # Borrado robusto para fechas en formato DD/MM/YYYY
                                patron = f"%/{mes_actual:02d}/{anio_actual}"
                                # cursor.execute("DELETE FROM gastos WHERE fecha_operacion LIKE ?", (patron,))  # Eliminado
                                # conn.commit()  # Eliminado
                                # print(f"Registros del mes {mes_actual:02d}/{anio_actual} eliminados antes de la inserción.")  # Eliminado
                                ...
                                try:
                                    ...
                                    pass  # Operación de BD eliminada
                                    # print("Inserción en BD eliminada")
                            except Exception as e:
                                    print(f"Error al insertar los gastos en la base de datos: {e}")
                                finally:
                                    pass  # Eliminado cierre de conexión BD
                        except Exception as e:
