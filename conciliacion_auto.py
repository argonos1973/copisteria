...
def buscar_coincidencias_automaticas(gasto):
    ...
    if tiene_numero_documento:
        ...
        numero_buscado = patron_numero.group(0)
        tipo_doc = 'factura' if numero_buscado.startswith('F') else 'ticket'
        
        if tipo_doc == 'factura':
            cursor.execute('''
                SELECT 'factura' as tipo, id, numero, fecha, total, estado, importe_cobrado
                FROM factura
                WHERE estado = 'C' AND numero = ? AND ABS(total - ?) <= ?
                AND id NOT IN (SELECT documento_id FROM conciliacion_gastos WHERE tipo_documento = 'factura' AND estado = 'conciliado')
            ''', (numero_buscado, importe, tolerancia))
        else:
            cursor.execute('''
                SELECT 'ticket' as tipo, id, numero, fecha, total, estado, importe_cobrado
                FROM tickets
                WHERE estado = 'C' AND numero = ? AND ABS(total - ?) <= ?
                AND id NOT IN (SELECT documento_id FROM conciliacion_gastos WHERE tipo_documento = 'ticket' AND estado = 'conciliado')
            ''', (numero_buscado, importe, tolerancia))
        
        for row in cursor.fetchall():
            coincidencias.append({
                'tipo': row['tipo'], 'id': row['id'], 'numero': row['numero'],
                'fecha': row['fecha'], 'importe': row['total'], 'estado': row['estado']
            })
    else:
        # Si NO hay número, buscar por fecha e importe
        ...
        cursor.execute('''
            SELECT 'factura' as tipo, id, numero, fecha, total, estado
            FROM factura
            WHERE estado = 'C' AND fecha BETWEEN ? AND ? AND ABS(total - ?) <= ?
            AND id NOT IN (SELECT documento_id FROM conciliacion_gastos WHERE tipo_documento = 'factura' AND estado = 'conciliado')
        ...
        for row in cursor.fetchall():
            ...
            coincidencias.append({
                'tipo': 'factura', 'id': row['id'], 'numero': row['numero'],
                'fecha': row['fecha'], 'importe': row['total'],
                'diferencia': abs(row['total'] - importe), 'score': score
            })
        
        # Buscar en tickets cobrados
        cursor.execute('''
            SELECT 'ticket' as tipo, id, numero, fecha, total, estado
            FROM tickets
            WHERE estado = 'C' AND fecha BETWEEN ? AND ? AND ABS(total - ?) <= ?
            AND id NOT IN (SELECT documento_id FROM conciliacion_gastos WHERE tipo_documento = 'ticket' AND estado = 'conciliado')
        ''', (fecha_inicio, fecha_fin, importe, tolerancia))
        ...
        for row in cursor.fetchall():
            score = calcular_score(gasto, dict(row))
            coincidencias.append({
                'tipo': 'ticket', 'id': row['id'], 'numero': row['numero'],
                'fecha': row['fecha'], 'importe': row['total'],
                'diferencia': abs(row['total'] - importe), 'score': score
            })
