...
def validar_facturas_xsig_hoy():
    ...
    if not facturas:
        logger.info(f"No se encontraron facturas XSIG generadas hoy ({fecha_actual}).")
        return []
    
    logger.info(f"Se encontraron {len(facturas)} facturas XSIG generadas hoy ({fecha_actual}).")
    ...
    for idx, factura in enumerate(facturas, 1):
        ...
        if es_valida:
            facturas_validas += 1
            logger.info(f"  ✓ Factura válida y firmada: {es_firmada}")
            ...
            try:
                ...
                if datos_verifactu.get('correcto', False):
                    ...
                    logger.info("  ✓ Datos VERI*FACTU generados correctamente")
                    logger.info(f"    - Hash: {datos_verifactu.get('hash', '')[:16]}...")
                    resultado['datos_verifactu'] = datos_verifactu
                else:
                    logger.info(f"  ✗ Error al generar datos VERI*FACTU: {datos_verifactu.get('mensaje', '')}")
                    resultado['datos_verifactu_error'] = datos_verifactu.get('mensaje', '')
            except Exception as e:
                logger.info(f"  ✗ Excepción al generar datos VERI*FACTU: {str(e)}")
                resultado['datos_verifactu_error'] = str(e)
        else:
            logger.info(f"  ✗ Factura con errores: {', '.join(resultado_validacion.get('errores', []))}")
    ...
    logger.info(f"Total de facturas XSIG de hoy: {len(facturas)}")
    logger.info(f"Facturas válidas: {facturas_validas} ({facturas_validas/len(facturas)*100 if facturas else 0:.1f}%)")
    logger.info(f"Facturas con errores: {len(facturas) - facturas_validas} ({(len(facturas) - facturas_validas)/len(facturas)*100 if facturas else 0:.1f}%)")
    logger.info(f"Facturas procesadas correctamente: {facturas_procesadas} ({facturas_procesadas/len(facturas)*100 if facturas else 0:.1f}%)")
