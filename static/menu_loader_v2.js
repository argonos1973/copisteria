...
async function verificarSesionYCargarMenu() {
    try {
        ...
        // LIMPIAR CACHÉ DE SUSCRIPCIÓN - siempre verificar fresco
        sessionStorage.removeItem('aleph70_subscription');
        
        // 1. OBTENER DATOS DE SESIÓN (siempre fresco para suscripción)
        ...
        console.log('[MENU] Sesión obtenida, empresa_id:', sessionData.empresa_id);
        ...
        // IMPORTANTE: Esperar a que termine la verificación ANTES de cargar el menú
        const suscripcionBloqueada = await verificarSuscripcion(sessionData);
        if (suscripcionBloqueada) {
            console.log('[MENU] Acceso bloqueado por suscripción - no se carga el menú');
            return; // NO cargar el menú si está bloqueado
        }
...
// Retorna TRUE si el acceso debe ser bloqueado, FALSE si permitido
async function verificarSuscripcion(sessionData) {
    try {
        if (!sessionData || !sessionData.empresa_id) {
            console.log('[SUBSCRIPTION] No hay empresa_id en sesión');
            return false; // Sin empresa, permitir por ahora
        }
        ...
        if (!response.ok) {
            console.log('[SUBSCRIPTION] Error en respuesta:', response.status);
            return false;
        }
        
        const sub = await response.json();
        console.log('[SUBSCRIPTION] Estado recibido:', sub);
        ...
        // IMPORTANTE: Si days_remaining <= 0, el trial ha expirado - BLOQUEAR ACCESO
        const estadosBloqueados = ['expired', 'canceled', 'past_due', 'none', 'inactive'];
        
        if (sub.status === 'free_trial' && sub.days_remaining <= 0) {
            console.log('[SUBSCRIPTION] Trial expirado - bloqueando acceso. Días:', sub.days_remaining);
            mostrarBannerExpirado();
            bloquearAccesoApp();
            return true; // BLOQUEADO
        } else if (estadosBloqueados.includes(sub.status)) {
            console.log('[SUBSCRIPTION] Sin suscripción activa - bloqueando acceso. Estado:', sub.status);
            mostrarBannerExpirado();
            bloquearAccesoApp();
            return true; // BLOQUEADO
        } else if (sub.status === 'free_trial' && sub.days_remaining > 0) {
            console.log('[SUBSCRIPTION] Trial activo. Días restantes:', sub.days_remaining);
            mostrarBannerTrial(sub.days_remaining);
            return false; // Permitido
        } else if (sub.status === 'active' || sub.status === 'trialing') {
            console.log('[SUBSCRIPTION] Suscripción activa');
            return false; // Permitido
        }
...
// Bloquear completamente el acceso a la aplicación
function bloquearAccesoApp() {
    console.log('[SUBSCRIPTION] Bloqueando acceso a la aplicación');
    
    // Deshabilitar todos los enlaces del menú
    document.querySelectorAll('.menu-link, .submenu-item, .submenu-link').forEach(link => {
    ...
    // Bloquear todas las peticiones fetch
    const originalFetch = window.fetch;
    window.fetch = function(url, options) {
        // Permitir solo rutas de suscripción, logout y assets
        const urlStr = url.toString();
        if (urlStr.includes('/api/subscription') || 
        ...
        console.log('[SUBSCRIPTION] Petición bloqueada:', urlStr);
        return Promise.reject(new Error('Suscripción expirada'));
    };
    ...
    document.addEventListener('keydown', function(e) {
        ...
        // Bloquear otras combinaciones de teclado
    });
}
