...
def importar_desde_excel(ruta_excel: str):
    ...
    # (fecha_operacion, concepto, importe_eur) como pseudo-clave única.
    cursor.execute("SELECT fecha_operacion, concepto, importe_eur FROM gastos")
    raw_registros = cursor.fetchall()

    def _normalize(fecha, concepto, importe):
        ...
        # Normalizar concepto (mayúsculas + sin espacios extremos)
        ...
        return (fecha_str, concepto_norm, importe_norm)

    registros_existentes = set(_normalize(f, c, i) for f, c, i in raw_registros)
    ...
    df['concepto_cmp'] = df['concepto'].astype(str).str.strip().str.upper()
    ...
    mask_nuevos = ~df.apply(lambda row: (row['fecha_cmp'], row['concepto_cmp'], row['importe_cmp']) in registros_existentes, axis=1)
    nuevos_registros = df[mask_nuevos].drop(columns=['fecha_cmp', 'concepto_cmp', 'importe_cmp'])

    inserted = 0
    if not nuevos_registros.empty:
        nuevos_registros.to_sql('gastos', conn, if_exists='append', index=False)
        inserted = len(nuevos_registros)
