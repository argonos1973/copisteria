...
def main():
    logger.info("=== VALIDACIÓN DE FACTURAS XSIG ===")
    ...
    logger.info(f"Se encontraron {len(facturas)} facturas con archivos XSIG")
    ...
    for i factura in enumerate(facturas, 1"):
        ...
        try:
            ...
            if es_valida:
                try:
                    ...
                    if datos_verifactu:
                        ...
                        logger.info(f"    - Hash: {datos_verifactu['hash'][:16]}...")
                except Exception as e:
                    logger.info(f"  ✗ Error al generar datos VERI*FACTU: {str(e)}")
            
            # Mostrar resultado
            if es_valida:
                validadas += 1
                logger.info(f"  ✓ Factura válida y firmada: {resultado.get('firmado', False)}")
            else:
                con_errores += 1
                logger.info(f"  ✗ Factura con errores: {', '.join(resultado.get('errores', ['Error desconocido']))}")
            ...
            con_errores += 1
            logger.info(f"  ✗ Error al validar la factura: {str(e)}")
            resultados.append({
                'id': factura['id'],
                'numero': factura['numero'],
                'ruta_xml': factura['ruta_xml'],
    ...
    logger.info(f"Total de facturas XSIG: {total}")
    logger.info(f"Facturas válidas: {validadas} ({validadas/total*100:.1f}%)")
    logger.info(f"Facturas con errores: {con_errores} ({con_errores/total*100:.1f}%)")
    logger.info(f"Facturas procesadas correctamente: {procesadas_ok} ({procesadas_ok/total*100:.1f}%)")
